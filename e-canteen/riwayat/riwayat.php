                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full bg-white">
                            ...
                            $query= mysqli_query($conn, "SELECT t.*, u.nama_user FROM top_up t 
                                      JOIN user u ON t.id_user = u.id_user 
                                      ORDER BY t.tanggal_transaksi DESC
                                      LIMIT $mulai, $batas");
                            while($trans = mysqli_fetch_array($query)) {
                                $statusClass = $trans['status'] == 'completed' ? 'text-green-600' : 
                                            ($trans['status'] == 'failed' ? 'text-red-600' : 'text-yellow-600');
                                echo "<tr class='border-b hover:bg-gray-50'>
                                        <td class='py-3 px-4 text-center border border-gray-400'>#" . str_pad($trans['id_topup'], 6, '0', STR_PAD_LEFT) . "</td>
                                        <td class='py-3 px-4 border border-gray-400'>{$trans['tanggal_transaksi']}</td>
                                        <td class='py-3 px-4 border border-gray-400'>{$trans['nama_user']}</td>
                                        <td class='py-3 px-4 border border-gray-400'>Rp " . number_format($trans['saldo'],0,',','.') . "</td>
                                        <td class='py-3 px-4 border border-gray-400 {$statusClass}'>{$trans['status']}</td>
                                        <td class='py-3 px-4 text-center border border-gray-400'>
                                            <button onclick='printReceipt({$trans['id_topup']}, \"{$trans['nama_user']}\", {$trans['saldo']}, \"{$trans['tanggal_transaksi']}\")' 
